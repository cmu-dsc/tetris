...
def train_dataset(datasets):
    ...
    while c < 200:
        ...
        for state, value in valid_loader:
            ...
            value = value.float()
            e_value = value - predicted_value
            loss_func = torch.nn.SmoothL1Loss()
            #loss_func = torch.nn.MSELoss()
            loss = loss_func(input=predicted_value, target=value)
            #loss = torch.mean(torch.log(torch.cosh(e_value + 1e-12))) # logcosh
            #loss = torch.mean(2 * e_value / (1 + torch.exp(-e_value)) - e_value) # xsigmoid
            #loss = torch.mean(e_value * torch.tanh(e_value)) # xtanh
