...
model = torch.load("value_net_nosigmoid.pth").cuda()
model.eval()
...
while not pc._game_over:
    gs = pc.gamestate()
    action = tree.search(pc, num_iter=200)
    if action == 0:
        pc.move_left()
    elif action == 1:
        pc.move_right()
...
if total_reward > highest_reward:
    ...
    for c in range(count):
        ...
        images.append(im)
    images[0].save("games/%s.gif" % "MCTS", save_all=True, append_images = images[1:],loop=0,duration=1)
