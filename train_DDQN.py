...
writer = SummaryWriter("runs/DDQN-0.001-0.9975_sparse_small")


gamma = 0.95
num_episode = 1000000000
pool_size = 10
model = ANN(216, 8, [256, 512])
model_target = ANN(216, 8, [256, 512])
...
for e in range(num_episode):
    ...
    while not pc._game_over and step < max_step:
        ...
        if len(replay_buffer) >= batch_size and counter > 10:
            ...
            target_Q = r + gamma * model_target(ns).gather(1, torch.max(model(ns), dim=1)[1].unsqueeze(1)).squeeze(1) * (1-d)
    ...
    torch.save(model, "DDQN_sparse.pth")
